...
Choosing one is partly a matter of taste and fit for the job at hand - choosing a fully featured template or something simpler for a simple task. Avoid unmaintained templates. Consider license implications for building on top of a template, lack of a license adds a risk. I've added a star ⭐ to ones that are particularly worth a look. It's hard to say whether it's better to take the kitchen-sink template, or one withe bare-minimum that you need and build on that, or to build your own personal template from one of them or from scratch. Horses for courses, and engineering tradeoffs. Apply brain here. There's lots here to build on. Good luck and enjoy! There's something especially pleasing about a great command line tool, and rust and clap make them fast and ergonomic.
